...
/**
 * "同一网络连续用不存在的手机号连续尝试"风险识别
 * @author ckwl
 */
@Component
@Slf4j
public class DeviceNetworkMobileAttemptRisk implements IUserRisk {
    ...
    public RiskLevel check(Object params, String factor) {
        ...
        // 获取不到本次登录的ip信息
        if(StringUtil.isBlank(userRiskPolicyArgs.getLoginIpAddress())){
            return RiskLevel.RISK_NONE;
        }

        Object count = stringCache.get(RedisKeyUtil.getLoginIpUserNotExistCount(userRiskPolicyArgs.getLoginIpAddress()));
